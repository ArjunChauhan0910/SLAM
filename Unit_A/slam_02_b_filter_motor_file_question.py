...
def filter_step(old_pose, motor_ticks, ticks_to_mm, robot_width,
    ...
    if motor_ticks[0] == motor_ticks[1]:
        ...
        d =  motor_ticks[0]*ticks_to_mm
        theta = old_pose[2]
        x = old_pose[0]+d*cos(theta)
        y = old_pose[1]+(d)*sin(theta) #since it is offset in y dimension
       ...
       cx = old_x - (R+(robot_width/2))*sin(old_ang)
       cy = old_y - (R+(robot_width/2))*-cos(old_ang)
       theta = (old_ang+alpha)%(2*math.pi)
...
if __name__ == '__main__':
    ...
    pose = (1850.0, 1897.0+scanner_displacement, 213.0 / 180.0 * pi)
    #pose[1] = pose[1]+scanner_displacement
    # Read data.
    logfile = LegoLogfile()
