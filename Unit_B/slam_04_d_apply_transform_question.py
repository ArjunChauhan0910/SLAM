...
def find_cylinder_pairs(cylinders, reference_cylinders, max_radius):
    ...
    for i in range(len(cylinders)):
        ...
        for j in range(len(reference_cylinders)):
            dist = sqrt((cylinders[i][0]-reference_cylinders[i][0])**2+(cylinders[i][0]-reference_cylinders[i][0])**2)
...
def estimate_transform(left_list, right_list, fix_scale = False):
    ...
    # --->>> Insert your previous solution here.
    l_m = [tuple(np.subtract(l,lc)) for l in left_list]
    r_m = [tuple(np.subtract(r,rc)) for r in right_list]
    #r_m = list(set(right_list)-set(rc))

    cs, ss, ll, rr = 0.0,0.0,0.0,0.0
...
def correct_pose(pose, trafo):
    ...
    pose = list(pose)
    trafo = list(trafo)
    pose[0] = trafo[3] + pose[0]
    pose[1] = trafo[4] + pose[1]
    delta = atan2(trafo[2],trafo[1])
    pose[2] = pose[2] + delta
